...
from src.ingestion.fetch_data import _get_api_key, fetch_many, fetch_one_quote
...
class TestFetchOneQuote:
    ...
    def test_rate_limit_response(self, mock_get_key, mock_get):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"Note": "Start using premium broke boy"}
        mock_response.raise_for_status = Mock()
        mock_get.return_value = mock_response

        with pytest.raises(
            ValueError, match="Rate limit exceeded: Start using premium broke boy"
        ):
...
class TestFetchMany:
    ...
    @patch(
        "src.ingestion.fetch_data.time.sleep"
    )  # mock sleep so we don't have to wait 1000s to run test suite
        ...
        mock_fetch.side_effect = [
            {
                ...
                "change_percent": "+1.5%",
                "fetched_at": "2025-11-10T12:00:00+00:00",
            },
            {
                "symbol": "GOOGL",
                "price": 2800.0,
                ...
                "timestamp": "2025-11-07",
                "change_percent": "+0.5%",
                "fetched_at": "2025-11-10T12:00:01+00:00",
            },
            {
                "symbol": "MSFT",
                "price": 380.0,
                ...
                "timestamp": "2025-11-07",
                "change_percent": "-0.3%",
                "fetched_at": "2025-11-10T12:00:02+00:00",
            },
        ]
